...
const isWithinMovementBoundaries = (x, y) => {
  if (
    RED_LINE[y] && RED_LINE[y].includes(x)
...
export const movePlayer = (keys, player) => {
  ...
  if (
    (keys.includes("ArrowUp") || keys.includes("KeyW")) &&
  ...
  if (
    (keys.includes("ArrowDown") || keys.includes("KeyS")) &&
    isWithinMovementBoundaries(absPlayerX, absPlayerY + PLAYER_SPEED)
  ) {
    playerMoved = true;
    player.y = player.y + PLAYER_SPEED;
  ...
  if (
    (keys.includes("ArrowLeft") || keys.includes("KeyA")) &&
  ...
  if (
    (keys.includes("ArrowRight") || keys.includes("KeyD")) &&
    isWithinMovementBoundaries(absPlayerX + PLAYER_SPEED, absPlayerY)
  ) {
    playerMoved = true;
    player.x = player.x + PLAYER_SPEED;
